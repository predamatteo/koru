...
class _OverlayDesignerScreenState extends ConsumerState<OverlayDesignerScreen> {
  ...
  Future<void> _save() async {
    final updated = _config.copyWith(
      ...
      messageSubtitle:
          _subtitleController.text.trim().isEmpty ? null : _subtitleController.text.trim(),
    );
  ...
  Widget build(BuildContext context) {
    if (!_loaded) _hydrate();
    final apps = ref.watch(installedAppsProvider).valueOrNull ?? const <InstalledAppInfo>[];
        ...
        children: [
          ...
          Wrap(
            ...
            children: _paletteHex.map((hex) {
              final selected = _config.backgroundColorHex.toUpperCase() == hex.toUpperCase();
              return GestureDetector(
                onTap: () =>
                    setState(() => _config = _config.copyWith(backgroundColorHex: hex)),
                child: Container(
                  width: 48,
                  height: 48,
                  decoration: BoxDecoration(
                    color: OverlayConfig(backgroundColorHex: hex).backgroundColor,
                    shape: BoxShape.circle,
                    border: selected
                        ? Border.all(color: KoruColors.textPrimary, width: 3)
                        : null,
                  ),
                ),
              );
          ...
          Slider(
            ...
            onChanged: (v) =>
                setState(() => _config = _config.copyWith(countdownSeconds: v.round())),
          ),
          SwitchListTile(
            contentPadding: EdgeInsets.zero,
            value: _config.allowBypassAfterCountdown,
            onChanged: (v) =>
                setState(() => _config = _config.copyWith(allowBypassAfterCountdown: v)),
            title: const Text('Allow opening after countdown'),
            subtitle: const Text('Show "Open anyway" button when countdown completes.'),
          ),
        ],
      ),
    );
